...
RUN cd /tmp && \
    ...
    ./config && make -j4 && make install && rm -rf /var/lib/apt/lists/*
...
# Build using CMake with static protobuf linking and hidden symbols
...
echo "OpenSSL 1.1.1b + protobuf 3.7.0 (STATICALLY LINKED) + CMake on ubuntu:jammy"\n\
echo ""\n\
echo "✅ Protobuf 3.7.0 is statically linked - no conflicts with system protobuf!"\n\
echo "✅ All protobuf symbols are hidden from dynamic linker"\n\
echo ""\n\
echo "Library dependencies (should NOT show protobuf):"\n\
